...
** 上下文与痛点
当通过 Emacs TRAMP 机制打开远程主机上的图片目录（例如使用 =image-dired= 浏览图片）时，由于 =image-dired= 在加载和渲染缩略图时会启动多个并行的 =convert= 异步子进程，这些进程频繁通过同一个或多个 SSH 隧道请求远程文件，容易在 Emacs 29+ 下触发 TRAMP 并发连接死锁（表现为 CPU 100% 占用，Emacs 界面完全卡死）。
